...
def main():
    ...
    parser.add_argument('--output', type=Path, default=base / 'evidence/zap-fixed-run')
    parser.add_argument('--zap-config', type=Path,
                        default=Path.home() / 'Library/Application Support/ZAP/config.xml')
    args = parser.parse_args()
    source = base / 'target/evidence-fixed'
    ...
    for label in ('baseline', 'oversized'):
        ...
        if status != expected['status'] or len(data.get('orders', [])) != expected['returnedOrders']:
            raise ValueError('ZAP did not reproduce the expected pagination behaviour.')
        if size != expected['responseBytes'] or data.get('totalItems', 0) != expected['totalItems']:
            raise ValueError('ZAP and Java client pagination evidence disagree.')
        ids.append({'finding': label, 'id': message['id'], 'status': status,
                    'ordersReturned': len(data.get('orders', [])), 'responseBytes': size})
    email_messages = [m for m in messages if m['requestHeader'].startswith('POST ')]
    if len(email_messages) != 6 or notifications['capturedEmails'] != 1:
        raise ValueError('Expected six notification messages and one captured email.')
    for index, message in enumerate(email_messages):
        status = int(message['responseHeader'].splitlines()[0].split()[1])
        payload = json.loads(message['requestBody'])
        if status != (429 if index == 5 else 200) or payload != {'email': 'vidura-test@example.test',
    ...
    if 'retry-after:' not in email_messages[-1]['responseHeader'].lower():
        raise ValueError('The blocked response must include Retry-After.')
    output = args.output.resolve()
    output.mkdir(parents=True, exist_ok=False)
    for finding in ('pagination', 'notifications'):
        shutil.copytree(source / finding, output / finding)
        shutil.copyfile(base / 'target' / (finding + '-fixed.log'), output / (finding + '-fixed.log'))
    ...
    summary = {
        ...
        'secureBaseCommit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=repo, text=True).strip(),
        'securedSourceSha256': fingerprints,
        'scope': 'Two listing requests and six notification requests. Secured application sources, synthetic data, temporary H2 database, local GreenMail SMTP catcher. No active scan or gateway test.',
        'limitations': 'This verifies page-size rejection, duplicate suppression and configured request throttling; it is not a load test or proof of complete denial-of-service resistance. No GUI Requester interaction or native screenshot was performed.',
    }
